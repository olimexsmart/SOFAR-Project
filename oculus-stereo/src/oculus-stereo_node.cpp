...
//OVR Include Files
#include "OVR.h"
#include "../include/CommonSrc/Platform/Platform_Default.h"
#include "../include/CommonSrc/Render/Render_Device.h"

#include "../include/CommonSrc/Render/Render_FontEmbed_DejaVu48.h"
#include "../include/CommonSrc/Platform/Gamepad.h"
...
public:
    ...
    void GrabFrame_left(const sensor_msgs::ImageConstPtr& msg);//get images
    void GrabFrame_right(const sensor_msgs::ImageConstPtr& msg);//get images
	...
	//CvCapture* capture;
	//IplImage lastFrame;
	cv::Mat lastFrame_left;
	cv::Mat lastFrame_right;
...
int FPViewer::OnStartup(int argc,const char** argv)
{
    ...
    left_image = image_receiver.subscribe("/left/image_raw",1,&FPViewer::GrabFrame_left,this);//listen for image
	right_image = image_receiver.subscribe("/right/image_raw",1,&FPViewer::GrabFrame_right,this);//listen for image
...
void FPViewer::GrabFrame_left(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    //lastFrame_left = ptr->image;
    //lastFrame = ptr->image.operator IplImage();
    flip(ptr->image, lastFrame_left,0);
}

void FPViewer::GrabFrame_right(const sensor_msgs::ImageConstPtr& msg)
{
    ROS_INFO("Received image");
    cv_bridge::CvImagePtr ptr;
    try
    ...
    //IplImage img = ptr->image;
    //cv::flip(ptr->image,ptr->image,-1);
    //lastFrame_right = ptr->image;
    //lastFrame = ptr->image.operator IplImage();
    flip(ptr->image, lastFrame_right,0);
}
...
{	
	...
	if(stereo.Eye == StereoEye_Left) {
	    //ROS_INFO("Rendering");
	    lastFrame=lastFrame_left;
	    ros::spinOnce();
		//GrabFrame();
	}
	else{
		lastFrame=lastFrame_right;
	}
