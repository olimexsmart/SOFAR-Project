...
namespace oculus_viewer
{
    ...
    void ImageDistortViewer::init()
    {
        ...
        private_node.param<std::string>("left_image", left_img, "/usb_cam/image_raw");
        private_node.param<std::string>("right_image", right_img, "/usb_cam/image_raw");
    ...
    void ImageDistortViewer::show()
    {
        if (use_display_)
        {
            if ((!right_.getImage().empty()) && (!right_.getImage().empty()))
            {
                viewer_.show(right_.getImage(), right_.getImage());
            }
        }
    }
